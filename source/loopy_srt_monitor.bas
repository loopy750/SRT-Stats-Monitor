...
'COMMON SHARED SetWindowPos() AS LONG
...
SUB __UI_OnLoad
    ...
    IF _FILEEXISTS(config_main) THEN
        ...
        DO
            ...
            IF LEFT$(file4, 1) <> "#" AND LEFT$(file4, 1) <> ";" AND LEFT$(file4, 1) <> "" THEN
                ...
                IF EqualFound THEN
                    ...
                    IF file4_var$ = "colorbackground" THEN LoopySRTMonitorBKG = VAL(file4_val$)
        ...
        ' ColorBackground
        IF LoopySRTMonitorBKG THEN
            SELECT CASE LoopySRTMonitorBKG
                CASE IS < 0
                    LoopySRTMonitorBKG = 0
                CASE IS > 200
                    LoopySRTMonitorBKG = 200
            END SELECT
            Control(LoopySRTMonitor).BackColor = _RGB32(LoopySRTMonitorBKG, LoopySRTMonitorBKG + 2, LoopySRTMonitorBKG + 6)
            Control(StatusLB).BackColor = _RGB32(LoopySRTMonitorBKG, LoopySRTMonitorBKG + 2, LoopySRTMonitorBKG + 6)
            Control(IndicatorLB).BackColor = _RGB32(LoopySRTMonitorBKG, LoopySRTMonitorBKG + 2, LoopySRTMonitorBKG + 6)
            Control(FileMenu).BackColor = _RGB32(LoopySRTMonitorBKG, LoopySRTMonitorBKG + 2, LoopySRTMonitorBKG + 6)
            Control(OptionsMenu).BackColor = _RGB32(LoopySRTMonitorBKG, LoopySRTMonitorBKG + 2, LoopySRTMonitorBKG + 6)
            Control(HelpMenu).BackColor = _RGB32(LoopySRTMonitorBKG, LoopySRTMonitorBKG + 2, LoopySRTMonitorBKG + 6)
        END IF
    ...
    IF CheckUpdateOnStartup = "true" AND iniFeatures = 0 THEN
        ...
        IF file224$ = Ver THEN verCheck$ = "This is the latest version (v" + _TRIM$(Ver) + ")..."
    END IF
    iniFeatures = 0
    ...
    SELECT CASE id
        ...
        CASE OptionsMenuAlwaysOnTop
            IF AlwaysOnTop <> 1 THEN
                AlwaysOnTop = 1
                y& = SetWindowPos&(Myhwnd, -1, 0, 0, 0, 0, &H2 + &H1 + &H40) ' Windows only
                SetRadioButtonValue OptionsMenuAlwaysOnTop
            ELSE
                AlwaysOnTop = 0
                y& = SetWindowPos&(Myhwnd, -2, 0, 0, 0, 0, &H2 + &H1 + &H40) ' Windows only
                SetRadioButtonValue OptionsMenuAlwaysOnTop
            END IF
...
SUB Timer01
    ...
    IF __MultiCameraSwitch = 0 THEN
        IF SceneLBActive THEN SetCaption Timer_Fail_Count_2LB, "Enabled" ELSE SetCaption Timer_Fail_Count_2LB, "Disabled" ' Displayed same time as others and not "IF __MultiCameraSwitch" above
    ELSE
        IF SceneLBActive THEN SetCaption Low_Bitrate_StatusLB, "Enabled" ELSE SetCaption Low_Bitrate_StatusLB, "Disabled" ' Displayed same time as others and not "IF __MultiCameraSwitch" above
    END IF
